...
export const COMPONENTS = {
  SERVICES_SUBSCRIBER: 'feathers services subscriber',
  SERVICES_PUBLISHER: 'feathers services publisher',
  SERVICES_EVENTS_PUBLISHER: 'feathers services events publisher',
  SERVICES_EVENTS_SUBSCRIBER: 'feathers services events subscriber',
  SERVICES_REQUESTER: 'feathers services requester',
  SERVICES_RESPONDER: 'feathers services responder'
}
...
export function isInternalService (app, serviceDescriptor) {
  // Default is to expose all services except healthcheck
  const healthcheckPath = stripSlashes(app.distributionOptions.healthcheckPath || '/distribution/healthcheck/')
  if (serviceDescriptor.path.startsWith(healthcheckPath)) return true
...
export function getDistributedServiceOptions (app, serviceDescriptor) {
  if (typeof app.distributionOptions.remoteServiceOptions === 'function') return app.distributionOptions.remoteServiceOptions(serviceDescriptor)
  else return null
}
