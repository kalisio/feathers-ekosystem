...
export class Service {
  constructor (options) {
    ...
    // setup allowed paths
    if (!options.allowedServicePaths) throw new Error('constructor: \'options.allowedServicePaths\' param must be provided')
    this.allowedServicePaths = _.castArray(options.allowedServicePaths).map(path => new RegExp(path))
    // setup working dir
  ...
  async import (data, params) {
    ...
    // retrieve the targeted service and check whether the path is allowed
    if (!data.servicePath) throw new Error('import: missing \'data.servicePath\'')
    const isPathAllowed = _.some(this.allowedServicePaths, path => path.test(data.servicePath))
    if (!isPathAllowed) throw new Error(`import: service path '${data.servicePath}' is not allowed`)
  ...
  async export (data, params) {
    ...
    const isPathAllowed = _.some(this.allowedServicePaths, path => path.test(data.servicePath))
    if (!isPathAllowed) throw new Error(`export: service path '${data.servicePath}' is not allowed`)
    const service = this.app.service(data.servicePath)
